...
- Even though you have a API Key the extension might reject it
    ...
    - The URL is not sanatized for the model, so it might be a problem a patch is in progress
    Fix:
...
- Select the text that you want to send to the API
### 1.3.0

```json
{
    // Specification for extension
    "model_used": "Changable (get the url from the website)",

    // Tokens generated by AI per exection
    // (Can be changed through settings)
    "token_generation": 128,
    "features": [
        "Add different model paths to use the specific model",
        "Text selections feature, to limit token consumption",
        "Select the text that you want to send to the AI",
    ]
